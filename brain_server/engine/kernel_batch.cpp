...
//
// fast_advance : the signature, arithmetic and operation order of fly-wirehead's kernel.cpp
//                memory_advance (stonkfly's MIT loop plus fly-wirehead's dopamine-gated
//                depression), rewritten for a different memory
...
extern "C" void fast_advance(
 ...
 // Same statements as memory_advance with v[i]->c.v, g[i]->c.g, adaptation[i]->c.adapt, ...
...
extern "C" void par_advance(
 ...
 auto body=[&](int r){
   Local& L=LS[r];
   // Same statements as fast_advance.
   auto evolve=[&](Cell& c,int64_t now,float current){
     int64_t d=now-c.last;if(d<=0)return;
     const int frozen=c.refr>0?c.refr-1:0;
     const int skip=(int)(d<frozen?d:frozen);
