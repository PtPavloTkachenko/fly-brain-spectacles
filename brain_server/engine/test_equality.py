...
package kernel, the worker's exact rgb_step call per 10 ms chunk).
    ...
    cd "${CYBERFLY_RUNTIME:-$HOME/cyberfly_runtime}/fly-wirehead" && uv run python "<repo>/brain_server/engine/test_equality.py" [--ms 300] [--kernels orig,fast]
...
def main():
    ...
    ap.add_argument("--kernels", default="orig,fast,par2,par3")
    args = ap.parse_args()
