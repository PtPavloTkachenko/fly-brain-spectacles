...
def cloud_subset(n):
    ...
    if CLOUD_FILE.exists():
        with np.load(CLOUD_FILE) as f:
            if len(f["idx"]) == n:
                return f["idx"]
            # 14.09: a server started with another --cloud silently REWROTE this file, and the lens's
            # BrainCloudData.ts (generated from it) no longer matched the bits it received: the
            # hologram lit the wrong neurons. The file is shared with the lens; never regenerate it.
            raise SystemExit(f"{CLOUD_FILE} has {len(f['idx'])} neurons, --cloud asks {n}: pass --cloud {len(f['idx'])} "
                             "(the lens's BrainCloudData.ts is generated from this file; see brain/export_cloud.py)")
...
async def main(args):
    ...
    for fly in range(args.flies):
        ...
        elif args.engine == "native":  # the shared C++ brain core (core/), one process per fly
            from engine.native_worker import run as native_run

            p = ctx.Process(target=native_run, args=(fly, to_worker, from_worker, args.flyb, args.step_ms, args.threads), daemon=True)
            p.start()
        else:
        ...
        try:
            async for raw in ws:
                ...
                for m in msgs:
                    ...
                    elif t == "memory":  # the plastic state as a blob (ADR 62; native engine only)
                        cmd = {"memory": m["op"]}
                        if "data" in m:
                            cmd["data"] = m["data"]
                        if "elapsed_s" in m:
                            cmd["elapsed_s"] = m["elapsed_s"]
                        if "compact" in m:
                            cmd["compact"] = m["compact"]
                        inboxes[fly].send(cmd)
                        log.info(f"fly {fly} memory {m['op']}" + (f" ({len(m['data'])} chars)" if "data" in m else ""))
                    elif t == "load":  # duty / pause / threads for the native core (the Python workers ignore it)
                        inboxes[fly].send({k: m[k] for k in ("duty", "pause", "threads") if k in m})
                    elif t == "learning":  # dopamine plasticity on/off (native engine; the Python workers ignore it)
                        inboxes[fly].send({"learning": bool(m.get("on"))})
                        log.info(f"fly {fly} learning {'ON' if m.get('on') else 'off'}")
                    elif t == "dbg":
                        rows = " | ".join(
...
if __name__ == "__main__":
    ...
    ap.add_argument("--engine", choices=["process", "batch", "native"], default="process",
                    help="native = core/, the shared C++ brain core (needs --flyb)")
    ap.add_argument("--flyb", default=str(HERE.parent / "core/brain_export/out/brain_c0.flyb"),
                    help="native engine: brain file from core/brain_export/export.py")
    ap.add_argument("--threads", type=int, default=2, help="native engine: kernel threads (2 = the exact partitioned kernel)")
