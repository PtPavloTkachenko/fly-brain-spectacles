...
Every mapping comes from `brain/atlas.py` (see docs/SENSES_AND_READOUTS.md "Decoder from the
...
SENSES = {
    ...
    # Hunger (12.09 the user: "wouldn't the brain itself call it to go and eat?"). It would not: the
...
def index_readouts(a):
    ...
    # 12.09 the user: "hook as much as possible up to the brain". The remaining motor pools, by
...
GROOM_RANGE_HZ = 12.0  # DNg12: bristles take it +8..+15 Hz over a ~1.4 Hz rest
...
def decode(rate, base):
    ...
    # ...and each leg on its own pool, so the six legs push independently (12.09 the user: "I thought
