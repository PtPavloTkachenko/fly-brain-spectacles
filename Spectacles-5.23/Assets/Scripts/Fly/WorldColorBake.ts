/**
 * WorldColorBake — the room's colours for the flies' eye cameras (11.09 Pavlo: "bake vertex colour
...
import { worldDeviceCamera } from "./WorldCameraId"

const log = new NativeLogger("WorldColorBake")
const TEX = 256
const mod = (a: number, n: number) => ((a % n) + n) % n
// 15.09 perf pass: CLAUDE.md's "no per-vertex vec3 in hot loops" — the projection call allocated one
// per vertex (up to BAKE_MAX_VERTS = 20,000 per bake) right under a comment saying it did not.
// project() / worldSpaceToScreenSpace() read the argument and return a fresh vec2; they never keep it.
...
export class WorldColorBake {
  ...
  /** The world mesh under `root`, resolved FRESH every time.
   *
   *  17.09, from the glasses: this used to run once in the constructor and cache the first mesh it
   *  found. On the device the tracker had not produced a room yet, so we latched onto its placeholder
   *  and kept it for the whole session: `verts=10000` (a 100x100 grid), `p0=0,0,0`, `behind=10000
   *  used=0`, `room=0x0x0`, `texels=0`, and - the giveaway - `cal=id? err=89/89/89/89`, four
   *  IDENTICAL candidate errors, which only a symmetric grid can produce. A real room is asymmetric
   *  and scored 265/60/27/294 the day before. The page then had no room to draw, correctly. */
  private findMesh(): RenderMesh | null {
    let found: RenderMesh | null = null
    const walk = (so: SceneObject) => {
      const r = so.getComponent("Component.RenderMeshVisual") as RenderMeshVisual
      if (r && r.mesh && !found) found = r.mesh
      for (let i = 0; i < so.getChildrenCount() && !found; i++) walk(so.getChild(i))
    }
  ...
  private pos: ArrayLike<number> = [] // Float32Array from getVertexDataForAttribute (5.23), read-only here
  private rowBuf: Uint8Array[] = [] // reused sparse-readback rows (15.09 perf pass)
  private geomT = 0
  private bakes = 0
  private texels = 0
  private err = ""
  ...
  constructor(worldMeshSource: SceneObject | null, material: Material | null, private cameraObject: SceneObject) {
    this.root = worldMeshSource
    this.src = this.findMesh()
  ...
  /** How many voxels carry a colour so far (grows through the scan). */
  get texelCount(): number {
    return this.texels
  }

  /** The baked colour of the voxel around a WORLD point in cm (the same texel mapping bake() writes
   *  with, sx/sz already applied by the caller): writes RGB into out[o..o+2] and returns true, or
   *  returns false and leaves out untouched when that voxel was never seen by the camera. */
  colorAtCm(px: number, py: number, pz: number, out: Uint8Array, o: number): boolean {
    ...
    const k = 4 * (t * TEX + u)
    if (this.pixels[k + 3] === 0) return false
    out[o] = this.pixels[k]
    out[o + 1] = this.pixels[k + 1]
    out[o + 2] = this.pixels[k + 2]
    return true
  }

  /** The voxel-hash colour texture (also painted onto the intro scan grid, RoomPaint). */
  get texture(): Texture {
    return this.tex
  }
  ...
  status(): string {
    ...
    return "verts=" + this.pos.length / 3 + " swaps=" + this.swaps + room + " " + this.calStr + " texels=" + this.texels + " bakes=" + this.bakes + " [" + this.lastStats + "]" + (this.err ? " err=" + this.err.substring(0, 60) : "")
  }

  // On the glasses the scan frame comes from the world colour camera, not the display camera, so the
  ...
  private colorCamera(): DeviceCamera | null {
    if (this.colorCam === undefined) {
      try {
        this.colorCam = worldDeviceCamera() // the camera WorldScanner resolved (the first one the runtime opens)
      } catch (e) {
        this.colorCam = null
      }
    }
  ...
  private updateBounds() {
    ...
    // from the flies -> 973 clamps in 25 s, the shoves Pavlo saw as teleports)
    const b = this.bounds
    this.bounds = b
  ...
  tick(dt: number, every = FlyConfig.BAKE_GEOM_S) {
    ...
    this.geomT = every
    // the tracker swaps the mesh under us once it has a real room: adopt it and forget what the
    // placeholder taught us (bounds, calibration, the painted voxels are all about the old frame)
    const now = this.findMesh()
    if (now && now !== this.src) {
      this.src = now
      this.swaps++
      this.bounds = null
      this.calLocked = false
      this.calWins = 0
      this.sx = 1
      this.sz = 1
      this.texels = 0
      ...
      this.count.fill(0)
      this.provider.setPixels(0, 0, TEX, TEX, this.pixels)
      log.i("BAKE_MESH_SWAP #" + this.swaps + ": the world mesh changed, calibration and paint reset")
    }
    if (!this.src) return
    try {
      ...
      // 5.23: getVertexDataForAttribute returns a Float32Array (extractVerticesForAttribute is deprecated since LS 336)
      const src: any = this.src
      this.pos = typeof src.getVertexDataForAttribute === "function" ? src.getVertexDataForAttribute("position") : src.extractVerticesForAttribute("position")
  ...
  bake(tex: Texture, camWorld: mat4) {
    ...
    try {
      ...
      // 15.09 perf pass: the 48 row buffers were allocated fresh on every bake (BAKE_ROWS x w x 4
      // bytes, ~150 kB a frame, every scan frame). They are the same size every time, so they are
      // allocated once and refilled in place. getPixels overwrites the whole row.
      if (this.rowBuf.length !== rows || (this.rowBuf.length > 0 && this.rowBuf[0].length !== w * 4)) {
        this.rowBuf = []
        for (let r = 0; r < rows; r++) this.rowBuf.push(new Uint8Array(w * 4))
      }
      const rowBuf = this.rowBuf
      for (let r = 0; r < rows; r++) {
        prov.getPixels(0, Math.min(h - 1, Math.floor(((r + 0.5) / rows) * h)), w, 1, rowBuf[r])
      }
      ...
      for (let v = 0; v < n; v += step) {
        ...
        if (cc) {
          ...
          PROJ.y = qy
          PROJ.z = qz
          s = cam.worldSpaceToScreenSpace(PROJ)
        }
