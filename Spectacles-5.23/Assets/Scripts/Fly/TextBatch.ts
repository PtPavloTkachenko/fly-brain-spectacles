 ...
 *
 * Ported from the author's MSDF text engine (written for an earlier Spectacles lens), minus the karaoke wipe:
...
class Face {
  ...
  /**
   * ADR 71: how wide this string would be, in the same cm a caller passes as `capCm`. A card can
   * check a label against its own width before it draws it off the edge (Pavlo's capture, 15.09:
   * the CC-BY credit ran past the card and under the GOT IT key).
   */
  widthOf(text: string, capCm: number): number {
    return this.laid(text).width * capCm
  }
...
export class TextBatch {
   ...
   *  inherits it, exactly as the original engine does ("bind the atlas on the SOURCE material once"). */
  ...
  private applyUniforms() {
    ...
    p.depthWrite = false
    p.depthTest = true // explicit: a 5.23 clone takes the .mat defaults (ADR 97)
    p.twoSided = true
  }

  /** ADR 97: the cards' text draws with their plates, under the fly */
  setRenderOrder(order: number) {
    this.vis.setRenderOrder(order)
  }
  ...
  /** ADR 71: how wide this string would draw, in the same cm `add` takes as `capCm`. */
  widthOf(text: string, capCm: number): number {
    return this.face.widthOf(text, capCm)
  }

  /**
   * How wide this string ACTUALLY draws, in board cm. 21.09: `widthOf` returns EM-width x capCm, but
   * `flush` scales glyphs by capCm / capEm (see the `s` there), so the real drawn width is ~1/capEm
   * (about 1.43x) larger. Every card that measured with `widthOf` therefore thought its text was ~30%
   * narrower than it draws, and let lines run off the edge -- which is what a run of cards did today.
   * FlyAttention already divides by capEm by hand; this is that correction, named, for the shared fit.
   */
  drawnWidthOf(text: string, capCm: number): number {
    const em = this.face.capEm > 0.05 ? this.face.capEm : 0.7
    return this.face.widthOf(text, capCm) / em
  }
