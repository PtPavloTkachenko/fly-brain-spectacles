 ...
 * flight trail behind EVERY fly (Pavlo 11.09), and a glow under the selected fly.
...
// 15.09 perf pass: reused scratch — setWorldPosition / quat.lookAt copy what they are given
const SCRATCH_POS = new vec3(0, 0, 0)
const SCRATCH_DIR = new vec3(0, 0, 1)
...
class Trail {
  ...
  update(dt: number, flyPos: vec3, camPos: vec3, widthCm: number) {
    ...
    this.lastW = widthCm
    // 12.09 Pavlo ("let it be less smooth but optimised"): the whole vertex buffer was re-uploaded
  ...
  private writePoint(i: number, flyPos: vec3, camPos: vec3, widthCm: number) {
    ...
    const u = i / (n - 1)
    const w = widthCm * u * u * (3 - 2 * u) // tapers to a fine point at the tail (11.09 Pavlo: "thin end")
    const a = this.va
    const b = this.vb
    a[0] = p.x + sx * w
    a[1] = p.y + sy * w
...
export class FlyFx {
  ...
  constructor(root: SceneObject, quad: RenderMesh, base: Material, flyCount: number) {
    // 11.09 Pavlo: a halo behind EVERY fly (its own colour); the selected one bright
  ...
  update(dt: number, flyPositions: vec3[], camPos: vec3) {
    ...
    for (let i = 0; i < flyPositions.length; i++) {
      ...
      if (!g) continue
      // soft glow plane UNDER the fly (11.09 Pavlo: "the glow plane under the fly, a general
      ...
      const gAny0 = g as any
      // 15.09 perf pass: the disc's position must follow the fly, but its billboard rotation only
      // changes when the DIRECTION to the head does. Both were rebuilt (vec3 + quat.lookAt) and
      // written every frame for every fly. The 0.002 gate is ~0.1 deg of turn.
      if (gAny0.px !== p.x || gAny0.py !== gy || gAny0.pz !== p.z) {
        gAny0.px = p.x
        gAny0.py = gy
        gAny0.pz = p.z
        SCRATCH_POS.x = p.x
        SCRATCH_POS.y = gy
        SCRATCH_POS.z = p.z
        gt.setWorldPosition(SCRATCH_POS)
      }
      const nx = len > 0.001 ? dx / len : 0
      const ny = len > 0.001 ? dy / len : 0
      const nz = len > 0.001 ? dz / len : 1
      if (Math.abs(nx - gAny0.nx) + Math.abs(ny - gAny0.ny) + Math.abs(nz - gAny0.nz) > 0.002 || gAny0.nx === undefined) {
        gAny0.nx = nx
        gAny0.ny = ny
        gAny0.nz = nz
        SCRATCH_DIR.x = nx
        SCRATCH_DIR.y = ny
        SCRATCH_DIR.z = nz
        gt.setWorldRotation(quat.lookAt(len > 0.001 ? SCRATCH_DIR : FWD, UP))
      }
