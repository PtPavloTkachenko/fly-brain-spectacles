...
> This is the **Mac brain server's** kernel (`brain_server/`), the original brain and today the editor bench. The web page runs the same model on a WebGPU kernel in `core/src/gpu/brain.wgsl`, whose exactness story is `core/DETERMINISM.md` and ADR 82.

`brain_server/engine/metal/` runs every fly's MaleCNS brain on the Apple GPU. Like the rest of CyberFly, it was vibe coded with Claude Code: we set the goal (same spikes, faster), and the agent profiled, built, tested and measured until the tests below passed. It is **bit-exact**: the same spikes and the same state as the CPU kernel it replaces, so a fly on the GPU is the same fly. On an M1 Max it is 1.5 to 6 times faster than our best CPU kernel (5.8x for one fly, 1.5x for five), and one fly runs faster than real time.
