/**
 * FlyEyes — each fly's own view of the room (11.09 the user: "the fly can have its own world
...
export class FlyEyes {
  ...
  private dots: { q: number; target: vec3 | null; pos: vec3 | null }[] = []
  ...
  constructor(cameraObject: SceneObject, sink: QuadSink | null) {
    ...
    if (sink) {
      ...
      for (let i = 0; i < RAYS.length * FlyConfig.FLY_COUNT; i++) {
        ...
        this.dots.push({ q: q, target: null, pos: null })
      }
      sink.flush()
    }
  }
  ...
  tick(dt: number, cam: Transform, selected: number) {
    ...
    for (let i = 0; i < this.dots.length; i++) {
      ...
      sink.set(d.q, {
        x: d.pos.x, y: d.pos.y, z: d.pos.z, visible: true,
        color: FlyConfig.FLY_COLORS[fly % FlyConfig.FLY_COLORS.length],
        intensity: fly === selected ? 1 : 0.55,
      })
    }
    sink.flush()
  }
