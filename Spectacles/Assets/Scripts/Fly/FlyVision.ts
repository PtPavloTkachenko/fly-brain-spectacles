/**
 * FlyVision — a real retina for every fly (11.09 the user: "cameras on their heads, rendering into
...
interface Eye {
  ...
  armed: boolean // its camera rendered last frame: the render target is fresh, read it now
...
export class FlyVision {
  ...
  private makeEye(root: SceneObject): Eye {
    ...
    return { cam: cam, rt: rt, prev: null, buf: new Uint8Array(W * H * 4), flowL: 0, flowR: 0, retina: null, valid: false,
      g: new Float32Array(W * H), out: new Array(16 * 8 * 3), timer: 0, armed: false, elapsed: 0 }
  }
  ...
  tick(dt: number) {
    ...
    for (const e of this.eyes) {
      e.timer += dt
      // its camera rendered on the previous frame, so the target is fresh: read it and switch off
      if (e.armed) {
        e.armed = false
        e.cam.enabled = false
        try {
      ...
      // due: let the camera draw ONE frame, and read it on the next tick
      e.elapsed = e.timer
      e.timer = 0
      e.cam.enabled = true
      e.armed = true
  ...
  private sample(e: Eye, dt: number) {
    ...
    // Date.now(), NOT getTime(): getTime() is the frame clock and never advances inside a frame,
    // so it measured a flat 0.00 (12.09). Millisecond granularity is fine because these are summed
    // over many samples and divided in status().
    const t0 = Date.now()
    const copy = ProceduralTextureProvider.createFromTexture(e.rt)
    if (e === this.eyes[0]) {
      this.lastW = copy.getWidth()
      this.lastH = copy.getHeight()
    }
    ;(copy.control as ProceduralTextureProvider).getPixels(0, 0, W, H, e.buf)
    const t1 = Date.now()
    this.readMs += t1 - t0
    this.nTimed++
    const g = e.g
