...
export class BrainCloud {
  ...
  /** The real brain + VNC outline around the dots (11.09 the user: "the silhouette should read").
      ...
      for (const prefab of prefabs) {
        ...
        const visit = (o: SceneObject) => {
          for (const rmv of o.getComponents("Component.RenderMeshVisual") as RenderMeshVisual[]) {
            ...
            // multiply the material's own values (FlyHolo values stay the user's, ADR 17)
  ...
  update(dt: number, msg: BrainMsg | null, color: vec4, cam: Transform) {
    ...
    // ...then CLOUD_FACE_DEG about the vertical turns the side we look at (12.09 the user: 180 deg to us)
    const face = FlyConfig.CLOUD_SWAY * Math.sin(this.spin) + (FlyConfig.CLOUD_FACE_DEG * Math.PI) / 180
    const sway = quat.angleAxis(face, new vec3(0, 1, 0))
    at.setLocalRotation(sway.multiply(quat.angleAxis(Math.PI / 2, new vec3(1, 0, 0))))
